...
//        onDragAccepted()
    ...
    fun flip(
        ...
        if (!isFlipped) {
            launch { rotationY.animateTo(180f, tween(350)) }
            launch { onFlip() }
        } else {
            launch { rotationY.animateTo(0f, tween(350)) }
            launch { onFlip() }
        }
    }
...
fun Modifier.swipe(
    ...
    val stopDrag = remember { mutableStateOf(false) }
    clickable(
        interactionSource = MutableInteractionSource(),
        indication = null
    ) {
        state.flip(scope, isFlipped, onClick)
    }
        .pointerInput(Unit) {
            detectDragGestures(
                onDragEnd = {
